...
DynamicSegment::DynamicSegment(const char* className, size_t slotSIze, unsigned numOfSlots) :
	name(className), slotSIze(slotSIze), numOfSlots(numOfSlots), freeSlotHead(NULL), count(0)
{
	...
	pStartAddress = reinterpret_cast <char *>(dynamicSegment.allocate(this->segmentSize));
}
...
void DynamicSegment::finalize() {}

void DynamicSegment::allocateSlots() {
...
void DynamicSegment::free(void* pDeleted) {
	if (pDeleted == NULL) {
