...
#include "Config.h"
#define _TMemoryManager12_Id _GET_TESTCASE_UID(_ELayer_PTS1::_eTMemroyManager12)
#define _TMemoryManager12_Name "TMemoryManager"

#include "../../../13PTechnical/PMemoryManager/PMemoryManager.h"

class TMemoryManager12 : public PMemoryManager {

public:
	static size_t getMemorySize() {
		int szTotalMemory1 = sizeof(PMemoryManager) + sizeof(size_t);
		int szTotalMemory2 = sizeof(PMemoryVariable) + PTC12_SIZE_MEMORYBASE + 2 * sizeof(size_t);
		int szTotalMemory3 = sizeof(PMemoryEven) + PTC12_COUNT_EXCEPTION * sizeof(Exception) + 2 * sizeof(size_t);
		int szTotalMemory4 = sizeof(PMemoryEven) + PTC12_COUNT_LOG * sizeof(Log) + 2 * sizeof(size_t);
	...
	TMemoryManager12(
		...
		unsigned nClassid = _TMemoryManager12_Id,
		const char* pcClassName = _TMemoryManager12_Name)
		: PMemoryManager(
			szTotalMemory,
	...
	virtual void Initialize() {
		...
		BaseObject::s_pMemory = new("BaseObject::s_pMemory") PMemoryVariable(PTC12_SIZE_MEMORYBASE);
		BaseObject::s_pMemory->Initialize();
		this->Add((Memory*)BaseObject::s_pMemory);

		Exception::s_pMemory = new("Exception::s_pMemoryEven") PMemoryEven(sizeof(Exception), PTC12_COUNT_EXCEPTION);
		Exception::s_pMemory->Initialize();
		this->Add((Memory*)Exception::s_pMemory);

		Log::s_pMemory = new("Log::s_pMemoryEven") PMemoryEven(sizeof(Log), PTC12_COUNT_LOG);
